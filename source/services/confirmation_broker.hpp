...
// asks to mutate a guarded surface, the worker BLOCKS here while a modal is
// shown ON THE CONSOLE. The user answers on-device; the worker resumes with the
// result. The context switch (PC -> console -> PC) is itself the safety
// mechanism.
//
// This class is the pure, host-testable state machine behind that. It owns NO
// UI and NO libnx - the main loop drives it; the worker thread only calls ask()
// and waits.
//
// Design decisions:
//   - ONE pending request at a time. A second ask() while one is pending is
//     AUTO-DENIED immediately (no queue).
//   - TIMEOUT auto-deny: an unanswered request resolves to Denied after a
//     timeout so the worker can't hang forever.
//   - SHUTDOWN auto-deny: if the app/servers are tearing down while a request
//     is pending, the worker is released with Denied.
