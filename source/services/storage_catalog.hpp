...
public:
    ...
    /// MTP's StorageInfo must report REAL capacity for anything writable -
    /// advertising zero on a writable surface makes hosts refuse client-side
    /// before any request reaches the device.
    ...
    /// Listing such a surface produces a folder that errors on entry over FTP,
    /// and a storage whose enumeration fails over MTP - the two transports
    /// disagreed on probing, which is the drift this class exists to prevent.
    ///
    /// Non-Filesystem surfaces have no mount and are always available. Save
    /// Data is Filesystem-kind so the write guard recognises "save:" paths,
    /// but its top two levels are synthesized - probing "save:/" would hide it
    /// entirely, so it is exempt.
