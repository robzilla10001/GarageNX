...
private:
    ...
    enum class Step {
        ...
        // ── XCI (slice 4c) ───────────────────────────────────────────────────
        // A gamecard image is: signature, header, then a root HFS0 whose
        // entries are partitions ("update", "normal", "secure", "logo"). The
        // NCAs live in `secure`, which is potentially gigabytes downstream past
    ...
    /// begin(), because a stream has no other way to know: the MTP gate has
    /// already established that the extension is one we accept.
    enum class Format { Pfs0, Xci };

    static Format format_from_name(const std::string& name);
    ...
    /// Set an entry's flags from its name. Shared by every front-end: PFS0 and
    /// HFS0 differ in how entries are laid out, not in what an entry means. An
    /// XCZ is exactly an XCI whose secure partition holds .ncz files, so this
    /// is the whole of XCZ support.
    static void classify(StreamEntry& se);
    ...
    bool   parse_xci_head();
    bool   parse_xci_root_header();
    bool   parse_xci_root_table();
    bool   parse_xci_secure_header();
    bool   parse_xci_secure_table();
    /// Shared by both HFS0 headers: validate magic, stash count/string-table
    /// size, and arm the collection of the table that follows. `what` names the
    /// partition for error messages.
    bool   parse_hfs0_header(Step table_step, const char* what);
    /// Decode the HFS0 entry table sitting in m_blob. `data_start` is where the
    /// partition's data region begins; entries carry offsets relative to it.
    bool   decode_hfs0_table(uint64_t data_start, std::vector<StreamEntry>& out);
    ...
    uint32_t m_ent_count = 0;
    uint32_t m_str_size  = 0;

    /// Where `secure`'s data region begins, carried from XciRootTable to
    /// XciSecureTable. Every NCA offset in the secure HFS0 is relative to it.
    uint64_t m_secure_abs = 0;
