...
//     return already lands on HOME, and qlaunch re-reads its application
//     records so freshly installed titles show up without a manual relaunch.
//   - Sphaira / hbmenu (an NRO): runs in applet mode under nx-hbloader. A
//     plain return hands control back to hbloader, which shows hbmenu - NOT
//     HOME; the process must be terminated instead.
//
// The context is detected with appletGetAppletType(). See app_exit.cpp for
// the mechanism.
