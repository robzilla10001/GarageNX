...
//
// Mounts the console's internal NAND partitions as browseable stdio devices so
// the file-manager transports can list and PULL from them:
//   bis_user:    - the User partition (save data, some system files)
//   bis_system:  - the System partition (firmware/system files) - DANGEROUS
//
// READ-ONLY by policy (StorageCatalog marks both ReadOnly + OnDevice-confirm).
// Mounting only exposes them for reading; the write-guard is what blocks
// mutation. A wrong write to bis_system can brick the console, which is why
// NAND system is gated OFF by default (nand_system=false) and nothing here
// ever issues a write - we only mount and browse.
//
