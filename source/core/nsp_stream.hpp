...
//
// A title presented as a sequential byte stream of a complete NSP (PFS0),
// built on the fly from the installed NCAs - a PC client can download an
// installed game over FTP/MTP/HTTP the way DBI does, with no temporary file
// and only a few MB of RAM for a 15 GB title.
//
// FTP RETR, MTP GetObject and HTTP GET all read sequentially from offset 0, so
// one pull interface serves every transport. Random access is deliberately not
// supported - a virtual NSP is for downloading, not seeking.
//
// THREADING: an instance is owned and used by ONE transport worker thread and
// holds its ncm sessions open for its lifetime (same shape as the installer).
// Not the pattern that crashed the title listing - that was 126 rapid session
// open/close pairs plus decryption, in a loop, racing the main thread.
//
// The SD-card dump (Core::Dump::dump_title_to_nsp) is a thin wrapper over this,
// so there is exactly ONE implementation of NSP construction to keep correct.
