...
//
// Save data is NOT a filesystem that can be mounted once: each title's save is
// a separate filesystem mounted per (user, application) pair, so the top two
// levels are synthesized from account + save-data enumeration and only the leaf
// is a real mount.
//
// MOUNT LIFECYCLE - SINGLE SLOT: at most ONE save is mounted at a time.
// Touching a path under a different title unmounts the current one and mounts
// that; touching anything outside a title's folder releases the mount. The
// system allows only a limited number of mounted save filesystems, and one slot
// cannot leak more than one mount - the failure mode is a remount rather than
// an exhausted handle table.
...
///
/// Deletion is BY save_data_id, the only stable handle to the exact record
/// (nothing in the fs API takes an application_id for deletion). The save MUST
/// NOT be mounted when this is called. This is the single most destructive fs
/// call in the app, so it is isolated here, takes an explicit id, and logs its
/// Result. Signature confirmed against libnx fs.h and hardware-verified via
/// Save Manager's own Delete button.
