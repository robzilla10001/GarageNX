...
enum class Storage { BuiltIn, SdCard, GameCard, BuiltInSystem };   // NAND / SD / inserted cartridge / firmware
...
inline NcmStorageId to_ncm_storage_id(Storage s) {
    switch (s) {
        ...
        case Storage::BuiltIn:        return NcmStorageId_BuiltInUser;
        case Storage::BuiltInSystem:  return NcmStorageId_BuiltInSystem;
        default:                      return NcmStorageId_BuiltInUser;
    }
}
#endif

enum class TitleType { Application, Patch, AddOnContent, Other, SystemProgram, SystemData, SystemUpdate, BootImagePackage, BootImagePackageSafe };



// Hash/equality for NcmContentId (used by the firmware dump's dedup set).
// PLATFORM_SWITCH-only: NcmContentId is a libnx type and does not exist in the
// host test build - an unguarded definition here broke every host suite that
// transitively includes this header.
#ifdef PLATFORM_SWITCH
// Hash function for NcmContentId (used in unordered_set)
struct NcmContentIdHash {
    size_t operator()(const NcmContentId& id) const {
        // Simple hash by XORing all bytes
        size_t h = 0;
        for (int i = 0; i < 16; ++i) {
            h ^= (size_t)id.c[i] << ((i % 8) * 8);
        }
        return h;
    }
};

// Equality function for NcmContentId
struct NcmContentIdEqual {
    bool operator()(const NcmContentId& a, const NcmContentId& b) const {
        for (int i = 0; i < 16; ++i) {
...
inline uint64_t base_application_id(uint64_t program_id, TitleType type) {
    switch (type) {
        ...
        case TitleType::SystemProgram:
        case TitleType::SystemData:
        case TitleType::SystemUpdate:
        case TitleType::BootImagePackage:
        case TitleType::BootImagePackageSafe:
