...
- **64-bit object sizes (done):** MTP's `ObjectCompressedSize` and the data-container length are both 32-bit, so a >4 GiB object arrives with `0xFFFFFFFF` in both — which would have silently capped a large stream install at 4 GiB, the very limit the feature exists to defeat. Rather than implement MTP object properties to recover a 64-bit size, `StreamInstaller::container_size()` derives the true length from the PFS0 entry table (64-bit and exact) and `recv_install` adopts it as soon as the header lands — well before a large transfer nears the 4 GiB mark. Unit-tested exact at 764 MB, just under 4 GiB, just over 4 GiB, and a 14 GiB multi-entry container. **Open risk (untested):** when a payload is an exact multiple of the endpoint's max packet size the host appends a zero-length packet; `recv_install` treats a 0-byte read as end-of-data-phase, but the interaction with `complete()` exiting early has only been reasoned about, not observed.
- **Throughput:** ~18 MB/s installing vs ~36 MB/s for a plain file copy. `recv_install`/`recv_file_data` strictly alternate USB read and storage write, so each idles while the other works; double-buffering with a writer thread is the obvious next gain.
