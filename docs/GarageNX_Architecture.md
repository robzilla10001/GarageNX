...
- **Slice 4:** install storages (`sd_install` / `nand_install`), which needs the sequential-reader change noted below.
...
**Stream install (design note, for slice 4).** The installer already streams: it never materialises a file, pulling 4 MiB chunks through a `read` callback straight into an NCM placeholder, so the FAT32 4 GiB file-size ceiling does not apply. The constraint is that `ContentEntry.read(offset, buf, len)` is **random-access** while a USB/socket transport is **sequential-only**; stream install requires consuming the PFS0 strictly in arrival order without seeking backward.
...
**Remaining M6 work:** MTP slice 4 (install storages).
